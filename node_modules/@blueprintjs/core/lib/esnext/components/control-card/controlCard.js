...
export const ControlCard = React.forwardRef((props, ref) => {
    const { alignIndicator, checked: _checked, children: labelContent, className, controlKind, defaultChecked: _defaultChecked, disabled, inputProps, inputRef, onChange: _onChange, showAsSelectedWhenChecked, ...cardProps } = props;
    const { checked, onChange } = useCheckedControl(props);
    // use a container element to achieve a good flex layout
    const labelElement = React.createElement("div", { className: Classes.CONTROL_CARD_LABEL }, labelContent);
    const controlProps = {
        alignIndicator,
        checked,
        disabled,
    ...
    return (React.createElement(Card, { interactive: !disabled, className: classes, ref: ref, ...cardProps }, controlKind === "switch" ? (React.createElement(Switch, { ...controlProps })) : controlKind === "checkbox" ? (React.createElement(Checkbox, { ...controlProps })) : (props.children)));
});
